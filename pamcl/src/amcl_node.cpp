...
class AmclNode
{
  ...
  private:
    ...
    ros::Time curr_publish;

    // For deal with covariance of Laser odometry============
    double lcov_x, lcov_y, lcov_a, lx, ly, la;
   void lodom_callback(const nav_msgs::Odometry::ConstPtr& msg)
   {
     this->lcov_x = msg->pose.covariance[0];
     this->lcov_y = msg->pose.covariance[7];
     this->lcov_a = msg->pose.covariance[35];
     this->lx = msg->pose.pose.position.x;
     this->ly = msg->pose.pose.position.y;
     this->la = tf::getYaw(msg->pose.pose.orientation);
   }
    // For deal with covariance of Laser odometry============
	...
	private_nh_("~")
{
  ...
  lodom_sub = nh_.subscribe("lodom",100, &AmclNode::lodom_callback, this);
}
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    odom_->SetLodom(this->lx, this->ly, this->la, this->lcov_x, this->lcov_y, this->lcov_a);
  ...
  curr_publish = ros::Time::now();
  double dt = (curr_publish-last_publish).toSec();

  if(resampled || force_publication || dt > 1)
  {
    last_publish = curr_publish;

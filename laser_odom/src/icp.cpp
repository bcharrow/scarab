...
double icp_compute_error(const vector<const point2d_t*>& a, const vector<const point2d_t*>& b);

pose_t icp_align(const tree2d_t *tree, vector<point2d_t> &targets, pose_t pose, int iteration, double thr, double &err)
{
	double curr_err = 999999;
	double prev_err = 999999;
	pose_t pose_acc;
	//pose_acc.p.x = 0;  //Changed
	//pose_acc.p.y = 0;
	//pose_acc.yaw = 0;
	...
	for (int i = 0 ; i < iteration; i ++)
	{
		...
		curr_err = icp_compute_error(a,b);
		if (abs(curr_err - prev_err) < thr)
			break;
		prev_err = curr_err;
	}
	err = curr_err;
...
double icp_compute_error(const vector<const point2d_t*>& a, const vector<const point2d_t*>& b)
{
	...
	for (unsigned int i = 0; i < a.size(); i++)
	{
		...
		err += sqrt(x_diff*x_diff + y_diff*y_diff);
	}
	err /= a.size();
	return err;
}
